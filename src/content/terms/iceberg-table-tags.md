...
Unlike branches (which are mutable and advance with new commits), tags are immutable once created. A tag always points to the same snapshot it was created with, providing a stable, named reference for point-in-time access that survives [snapshot expiration](/terms/snapshot-expiration) and catalog operations.
