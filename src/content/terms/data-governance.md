...
These rules are implemented as automated data quality checks that run after every pipeline execution. dbt's testing framework, Great Expectations, and [Dremio](/terms/dremio)'s built-in data quality monitoring all provide mechanisms for codifying and executing quality rules. When a quality check fails, automated alerts notify the data engineering team and, if the failure is severe, prevent the bad data from advancing from the Silver layer to the Gold layer where it would affect production dashboards.
...
[Apache Polaris](/terms/apache-polaris)'s [credential vending](/terms/credential-vending) architecture provides one mechanism for enforcing consistent access control across engines: every engine's storage access is mediated through Polaris, which applies RBAC rules before vending credentials. Dremio's [Row-Level Security](/terms/row-level-security) policies restrict which rows a specific user can see within a virtual dataset, enforcing organizational data boundaries transparently for all downstream BI tools.

### Metadata Management and Cataloging

Metadata governance ensures that data is discoverable, understandable, and trustworthy. A well-governed lakehouse maintains a [data catalog](/terms/data-catalog) that documents every table's purpose, schema, data owner, update frequency, quality scores, and the lineage of how the table was produced. When a new data scientist joins the organization, they can browse the catalog to understand what data is available, who owns it, what it means, and how reliable it is, without needing to schedule meetings with the data engineering team.
...
Dremio's [Semantic Layer](/terms/semantic-layer) serves as the governance control plane for analytical access. Row-Level Security policies, [Column Masking](/terms/column-masking) rules (which automatically obscure PII columns for users without elevated access), and access auditing are all enforced centrally within Dremio, regardless of which BI tool or SQL interface the user employs to query the data.
