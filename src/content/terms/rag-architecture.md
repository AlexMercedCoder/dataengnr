...
When the employee asks the question, the system does not send it directly to the LLM. First, the system converts the question into a Vector Embedding and performs a Semantic Search against the company's internal Vector Database. The database searches through millions of private PDFs, HR manuals, and Slack messages, and *retrieves* the three paragraphs most highly relevant to the question.
