...
## [Dremio](/terms/dremio) Data Reflections
...
[Apache Iceberg](/terms/apache-iceberg) supports materialized views through the `CREATE MATERIALIZED VIEW` DDL available in Spark and [Trino](/terms/trino). Iceberg materialized views are stored as Iceberg tables with automatic staleness tracking: when the underlying base tables change (new snapshots are committed), the materialized view is marked as stale and refreshed on the next query or on a scheduled refresh cycle.

The combination of Iceberg materialized views (managed through the Iceberg catalog) and Dremio Data Reflections (managed through Dremio's [Semantic Layer](/terms/semantic-layer)) provides a two-tier acceleration architecture: Iceberg views handle pre-aggregated analytical patterns managed through the catalog, while Dremio reflections handle ad-hoc acceleration patterns detected from real query traffic.
