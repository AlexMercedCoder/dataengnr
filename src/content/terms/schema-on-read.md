...
Schema-on-read is the architectural philosophy that created the [Data Lake](/terms/data-lake). The rule is simple: Just dump the data.

When millions of complex, nested JSON files stream in from a mobile app, the data lake (e.g., an S3 bucket) does not check or enforce a schema. It simply stores the raw files exactly as they are. This allows organizations to ingest petabytes of [unstructured data](/terms/unstructured-data) instantly, without any upfront engineering effort.
