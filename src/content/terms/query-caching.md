...
In a modern lakehouse architecture utilizing [Apache Iceberg](/terms/apache-iceberg), cache invalidation is highly efficient. Because Iceberg provides strict snapshot IDs for every transaction, the query engine simply checks if the underlying table's snapshot ID has changed since the cache was created. If it hasn't, the cache is guaranteed to be perfectly accurate and is served instantly.
...
**1. Metadata Caching**: Caching the structural information about the tables (where the files are located on S3, column statistics). This prevents the engine from making slow API calls to the [object storage](/terms/object-storage) just to plan the query.
...
**3. [Result Set Caching](/terms/result-set-caching)**: The highest level of caching. Caching the final, computed output of the query so the entire execution plan can be bypassed.
