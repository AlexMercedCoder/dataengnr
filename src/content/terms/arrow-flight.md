...
Python clients connecting to Dremio through the `pyarrow.flight` library or the `dremio-simple-query` Python library receive query results as Arrow Tables directly. These Arrow Tables can be passed directly to pandas (`df = arrow_table.to_pandas()`), [Polars](/terms/polars) (`polars.from_arrow(arrow_table)`), or [DuckDB](/terms/duckdb) (`duckdb.execute("SELECT * FROM ?", [arrow_table])`), all with zero data copying because these libraries natively consume Arrow memory.
