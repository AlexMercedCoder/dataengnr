...
Dremio's semantic layer, with its REST API and Arrow Flight data delivery, provides the governed data access interface that makes agentic analytics safe and reliable in production. The semantic layer is the MCP (Model Context Protocol) data source for AI agent queries, providing governed, consistent data access without giving agents direct access to raw Iceberg tables.
